function Radar() {
	...
	function main(selection) {
		...
		selection.each(function () {
			...
			if (svg === undefined) svg = el.append('svg').attr('width', size).attr('height', size);
			...
			if (back === undefined) {
				...
				back.selectAll('.level').data(levels).enter().append('polygon').attr('class', function (d, i) {
				...
				gAxes.append("text").attr("class", "axis-text").text(function (d) {
					...
					if (Math.round(d.apex.x) < center.x) return true;else return false;
			...
			if (dataArea === undefined) dataArea = svg.append('g').attr('transform', function (d) {
				return "translate(" + margin.left + "," + margin.top + ")";
			});

			opacity = opacity.domain([0, dados.length]);
			var id = 0;
			var enterData = dataArea.selectAll('.entity').data(dados, function (d) {
			...
			var polygons = enterData.enter().append('polygon').attr('fill', function (d, i) {
				return 'none'; //color[i];
			}).attr('class', 'entity').attr('stroke', function (d, i) {
				return color[i];
			}).attr('fill-opacity', function (d, i) {
				if (fill) return opacity(i);else return 0;
			}).attr('stroke-width', "4px").attr("points", function (d) {
				...
				var polygon = points.map(function (el) {
					return [el.x, el.y];
				});
				...
				dataArea.selectAll('.centerpoint').data([center]).enter().append('text').attr('class', 'centerpoint').attr('x', function (d) {
					return d[0];
				}).attr('y', function (d) {
					return d[1];
				}).text(porcentagem(area, totalArea) + "%").style('text-anchor', 'middle');
				var dta = dataArea.selectAll('.points').data(points).enter();

				svg.selectAll('.axis-text').transition(duration).style('opacity', 0.5);
				dta.append('text').attr('class', 'points').attr('x', function (d) {
					return d.x < main.center().x ? d.x - 20 : d.x + 20;
				}).attr('y', function (d) {
					return d.y < main.center().y ? d.y - 20 : d.y + 20;
				}).classed('anchor-end', function (d) {
					//console.log(Math.round(d.apex.x));
			...
			polygons.merge(enterData).transition().duration(2000).attr('points', function (d) {
				return d.points.map(function (p) {
					return [p.x, p.y].join(",");
				}).join(" ");
			});
...
function calcArea(d) {
	var a = d3.polygonArea(d.attr('points').split(' ').map(function (el) {
